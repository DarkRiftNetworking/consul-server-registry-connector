...
namespace DarkRift.Server.Plugins.ServerRegistryConnectors.Consul.Tests
{
    /// <summary>
    ...
    public class ConsulServerRegistryConnectorIT
    {
        ...
        /// </summary>
        private Process consul;

        /// <summary>
        /// The URI consul is available on.
        /// </summary>
        private readonly string consulUri = Environment.GetEnvironmentVariable("CONSUL_URI") ?? "http://localhost:8500";

        [TestInitialize]
        public void Initialize()
        {
            // consul doesn't get killed after test scenarios fail or exit unexpectedly. If that's the case we'll use the old
            // one and reset it back to a clean slate.
            if (IsConsulRunning())
                ResetConsul();
            else
                StartConsul();
        ...
        private DarkRiftServer StartServer(string group, bool withHealthCheckAvailable = true)
        {
            ...
            serverSpawnData.ServerRegistry.ServerRegistryConnector.Type = nameof(ConsulServerRegistryConnector);
            serverSpawnData.ServerRegistry.ServerRegistryConnector.Settings.Add("consulAddress", consulUri);
        ...
        /// <summary>
        ///     Resets consul so it can be reused.
        /// </summary>
        private void ResetConsul()
        {
            string url = consulUri + "/v1/kv/?recurse=true";
        ...
        /// <summary>
        ///     Starts a new consul instance and waits for it to initialise.
        /// </summary>
        private void StartConsul()
        {
            consul = Process.Start("consul", "agent -dev");

            WaitUntil("Timed out waiting for consul to start.", () => Assert.IsTrue(IsConsulRunning()), TimeSpan.FromSeconds(10));
        }
        ...
        private bool IsConsulRunning()
        {
            ...
            {
                using WebClient client = new WebClient();
                client.DownloadString(consulUri);
            }
            catch
            {
                return false;
